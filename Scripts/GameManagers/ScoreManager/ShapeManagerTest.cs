...
public class ShapeManagerTest : MonoBehaviour
{
    ...
    [Header("Put Debug Level File Name Here")]
    [SerializeField] private string loadLevel;

    [Header("Show Potential Matches")]
    [SerializeField] private bool isShowPotential;
    void Awake()
    {
    ...
    private CombinationScoreCalculator scoreCalculator;
    ...
    public int CalculateMatchScore(IEnumerable<GameObject> matchedBlocks)
    {
        // 점수 계산기 생성 (추가 점수 및 추가 계수는 0으로 초기화)
        scoreCalculator = new CombinationScoreCalculator(combinationData, 0, 0.0f);

        // 점수 계산 및 반환
        return scoreCalculator.CalculateScore(matchedBlocks);
    }
    ...
    public void InitializeCandyAndSpawnPositionsFromPremadeLevel()
    {
        ...
        var premadeLevel = DebugUtilities.FillShapesArrayFromResourcesData(loadLevel);
